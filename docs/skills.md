...
- **Root Cause:** สองชั้นที่ผลักความรับผิดชอบหากันไปมา
  1. 🔴 **`SELECT *` ที่ต้นทาง** — ผู้เขียนเดิมตั้งใจแค่ "หาว่ามี user นี้ไหม" แต่การเขียนสั้น ๆ ด้วย `SELECT *` ดึงคอลัมน์ส่วนตัวทั้ง 20+ คอลัมน์มาโดยปริยาย แล้ว `dict(row)` ก็พาไปลง log ต่อ
  2. 🔴 **`AuditLogger` ไม่มีด่านอะไรเลย** — มัน serialize ทุกอย่างที่ผู้เรียกส่งมา (`json.dumps(old_values, default=str)`) ด้วยเหตุผลว่า "audit ต้องครบ" ⇒ ไม่มีชั้นใดรับผิดชอบเรื่องความเป็นส่วนตัว
  🔎 **จุดที่หลอกตาและทำให้รอดสายตามาได้:** `audit_logs` *เป็นตารางที่ควรมี* และ *การ log ครบถ้วนเป็นสิ่งที่ docs สั่งไว้* ⇒ ไม่มีใครอ่านโค้ดแล้วสะดุด เนื่องจาก "ทำตามกฎ" ทุกบรรทัด ความเสียหายอยู่ที่ **ที่เก็บ** ไม่ได้อยู่ที่การกระทำ
  🔎 **ทำไมแก้ที่ `SELECT *` อย่างเดียวไม่พอ:** ผู้เรียก `service_logger.log()` มี **31 จุด** และ `grep old_values=` โผล่ในทุกโมดูล ⇒ คนถัดไปที่เขียน `old_values=dict(row)` จะเปิดรูเดิมอีก การแก้ทีละผู้เรียกคือการพึ่งวินัยของคนที่ยังไม่เกิด
- **Correct Pattern/Solution:** แก้สองชั้น โดยชั้นที่สองคือชั้นที่รับประกันได้
  ```python
  # ชั้น 1 — ต้นทาง: ระบุคอลัมน์เท่าที่ใช้
  _IDENTITY_COLS = "id, email, google_id, discord_id"
  existing_by_provider = await conn.fetchrow(
      f"SELECT {_IDENTITY_COLS} FROM users WHERE ...")

  # ชั้น 2 — ปลายทาง (core/logger.py): ทุกคนต้องผ่านด่านนี้
  old_values = redact_sensitive(old_values)
  new_values = redact_sensitive(new_values)
  ```
  - `redact_sensitive()` ไล่ลงทั้ง dict/list ที่ซ้อน ⇒ ครอบทั้ง `{"existing_by_provider": {...}}` และ `{"items": [{...}]}`
  - ใช้ **`"[REDACTED]"` แทนการลบคีย์ทิ้ง** — audit ต้องยังตอบได้ว่า "มีคนแก้ฟิลด์นี้" ซึ่งเป็นสาระของ audit แค่ไม่ต้องรู้ค่า (ลบคีย์ทิ้ง = ทำให้ log ตอบคำถามเดิมไม่ได้)
  - ต้องมี **pattern match** ด้วย (`password|secret|token|api_?key|_hash$`) ไม่ใช่แค่รายชื่อคีย์ตายตัว ⇒ คีย์ที่ยังไม่เกิดวันนี้ (`refresh_token`, `student_password`) ถูกปกปิดอัตโนมัติ โดยไม่ต้องมีใครมาจำได้ว่าให้มาเพิ่มชื่อ
  - `redact_sensitive()` **ต้องไม่ mutate input** — ผู้เรียกหลายเจ้ายังใช้ค่าจริงต่อหลัง log (เช่น `link_oauth_account` เอา `old_user['phone_number']` ไป `COALESCE` ต่อ)

  **ล้างย้อนหลัง** — `backend/scripts/purge_audit_pii.py` เรียก `redact_sensitive()` **ตัวเดียวกับ logger** ไม่เขียนกฎซ้ำเป็น SQL เพราะกฎสองชุดจะเพี้ยนออกจากกันในอนาคต (และ `jsonb_set` เติมคีย์ที่ไม่มีอยู่จริงได้ ⇒ ต้อง `?|` กรองก่อน)
...
- **Correct Pattern/Solution:** ก่อน `docker stack deploy` ด้วยมือ **ทุกครั้ง**
  ...
  ```
  ⚠️ **ข้อความ error ห้ามมีเครื่องหมายขีด `-` เท่านั้น** — ถ้ามี docker จะ "ไม่ฟ้อง error" แต่ตัดข้อความที่ขีดตัวแรก แล้วเอาส่วนที่เหลือไปเป็นชื่อ tag เงียบ ๆ ซึ่งแย่กว่าเดิม (ทดสอบกับ `docker stack config` จริง 2026-09-26)
    🔴 **ข้อจำกัดมีข้อเดียว: ห้ามมีขีด `-` ที่ตำแหน่งใดก็ได้** — parser อ่าน `-` เป็นไวยากรณ์ `:-` แล้วถือว่าข้อความข้างหลังคือ *ค่า default* ⇒ `${IMAGE_TAG:?a-b}` ไม่ error แต่ได้ tag `b` / `${IMAGE_TAG:?run pull-all now}` ได้ tag `all now`
    ❌ **แก้ 2026-09-26 (แก้ตัวเอง):** เอกสารนี้เคยเขียนว่า "ต้องใส่เครื่องหมายคำพูดด้วย" — **ไม่จริง** ทดสอบซ้ำ 6 แบบแล้ว ข้อความที่ไม่มีขีด **ล้มดัง ๆ เสมอ** ไม่ว่าจะใส่คำพูดหรือไม่ (`:` `=` `*` `/` `.` และเว้นวรรคใช้ได้หมด) ⇒ คำพูดเป็นเรื่องรสนิยม ไม่ใช่ข้อบังคับ
    📌 ที่สำคัญคือ **ข้อความที่ใช้จริงต้องไม่มีขีด** — และของจริงใน `docker-compose.app.yml` ก็ไม่มี จึงทำงานถูก แต่คำอธิบายที่ผิดทำให้คนแก้ต่อใส่ขีดโดยคิดว่าคำพูดช่วยได้ ⇒ ต้องแก้ให้ตรง
  🧪 **จะทดสอบ compose ต้องใช้ `docker stack config` — ห้ามใช้ `docker compose config`** (พิสูจน์ 2026-09-26) เพราะสองตัวนี้ **อ่าน `.env` ไม่เหมือนกัน**:
  | คำสั่ง | อ่าน `.env` | ผลกับไฟล์เดียวกัน + `.env` ที่มีค่าครบ |
  |---|---|---|
  | `docker compose config` | ✅ อ่าน | `img-FROMDOTENV:FROMDOTENV` (ผ่าน) |
  | `docker stack config` | ❌ **ไม่อ่าน** | ล้ม: `required variable IMAGE_TAG is missing a value` |
  ⇒ production deploy ด้วย `docker stack deploy` ⇒ **`docker compose config` ให้คำตอบคนละอย่างกับที่เครื่องจริงเจอ** = "ผ่านในเครื่อง แต่ล้มบนเครื่องจริง" ซึ่งเป็นความผิดพลาดที่แพงที่สุดแบบหนึ่ง · การทดสอบที่ตรงคือ `docker stack config -c docker-compose.app.yml` แล้ว `export` ค่าเองเหมือนที่ `pull_all.sh` บรรทัด 5 ทำ (นี่คือเหตุที่ตอนทดสอบในโฟลเดอร์ชั่วคราว `ENV_NAME` ออกมาเป็นค่าว่าง)
...
### 🪤 ตัวตรวจที่ยิง "ใช้เครื่องมือนี้ได้ไหม" **ตอนที่กับดักยังติดอยู่** ⇒ ถอยไปใช้ทางสำรองเงียบ ๆ
- **Context/Problem:** เขียน `.github/workflows/ci-compose.yml` แล้วต้องเลือกว่า step ทดสอบจะ render compose ด้วย `docker stack config` (ตัวที่ production ใช้จริง) หรือถอยไป `docker compose config` (ตัวที่คิดว่า runner ใช้ได้) จึงเขียน probe ว่า `if docker stack config -c docker-compose.app.yml >/dev/null 2>&1; then …` **ก่อน** ที่จะ `export IMAGE_TAG`
- **Root Cause:** probe ถูกรันในสถานะที่ **กับดัก `${IMAGE_TAG:?...}` ยังไม่ถูกปลด** ⇒ `docker stack config` ล้ม **เพราะกับดักของเราเอง** ไม่ใช่เพราะเครื่องไม่มีความสามารถนั้น ⇒ ตัวแปร `RENDER` ตกไปที่ทางสำรอง **ทุกครั้ง** และยัง `echo` ข้อความที่ **อ้างความเท็จ** ว่า "docker stack config ใช้ไม่ได้กับ runner นี้"
  🔴 **จับได้เพราะรันบนเครื่องนี้ซึ่งเป็น swarm manager จริง** — ตามข้อความที่มันพิมพ์ มันควรใช้ `docker stack config` ได้ แต่กลับถอยไปใช้ตัวสำรอง ⇒ ความผิดพลาดแบบนี้ **มองไม่เห็นจากผลลัพธ์** (CI เขียวทั้งสองทาง) เห็นได้จาก **ข้อความที่มันรายงาน** เท่านั้น
  ⚠️ **โทษคือ "ทดสอบผิดตัว"** — ทั้ง workflow จะการันตีความถูกต้องของ compose ด้วย engine คนละตัวกับที่ production ใช้ ทั้งที่สามารถใช้ตัวจริงได้ · เป็นตระกูลเดียวกับ `docker compose config` vs `docker stack config` ที่อ่าน `.env` ไม่เหมือนกัน (หัวข้อข้างบน)
- **Correct Pattern/Solution:** ตั้งค่าให้ครบก่อน probe — ย้าย `export IMAGE_TAG=ci00000` ขึ้นไป **เหนือ** บล็อก `if` แล้วค่อย `unset` ทีหลังสำหรับเทสต์เคส "ไม่มี IMAGE_TAG"
  ✅ **ผลพลอยได้ที่แก้สมมติฐานเดิมของผม:** พอ probe ทำงานถูก **GitHub runner ก็ใช้ `docker stack config` ได้** (log: `ℹ️ ใช้ docker stack config (ตรงกับ production)`) ⇒ ข้อสันนิษฐานว่า "runner ไม่ได้อยู่ใน swarm mode จึงใช้ไม่ได้" **ไม่จริง** — ทางสำรองจึงไม่เคยถูกใช้เลย และ CI ได้ทดสอบด้วยคำสั่งเดียวกับ production จริง ๆ
- **Rule:** (1) 🔴 **probe ความสามารถของเครื่องมือ ต้องรันในสถานะที่ "ทุกอย่างพร้อม"** — ถ้าสิ่งที่กำลังทดสอบทำให้ probe ล้มได้เอง ผลลัพธ์จะเป็น false negative ที่ **ดูเหมือนข้อจำกัดของ environment** (2) 🔴 **`if cmd >/dev/null 2>&1` ทิ้งสาเหตุของความล้มเหลวทั้งหมด** ⇒ ถ้าผลของ probe เปลี่ยนเส้นทางที่เหลือของงาน ต้องเก็บ stderr ไว้โชว์อย่างน้อยตอนที่มันล้ม (3) ⚠️ **ข้อความ `echo` ที่อ้างเหตุผลต้องมีคนตรวจ** — "ใช้ไม่ได้กับ runner นี้" เป็นคำกล่าวอ้างเกี่ยวกับโลก ไม่ใช่คำอธิบายโค้ด
- **Tests:** ทดสอบ 4 กรณีกับไฟล์ที่จงใจทำให้พัง (ลบกับดัก / ใส่ขีด / เพิ่ม service ที่ 4 / ต้นฉบับ) แล้วยืนยันว่า CI บน runner จริงเลือก `docker stack config` — ดู PR #89
- **Date Added:** 2026-09-26

### 🔍 `grep` ทั้งไฟล์บน config ที่ **เอกสารตัวเอง** ⇒ นับเกิน เพราะคอมเมนต์มีแพตเทิร์นจริงอยู่
- **Context/Problem:** ตัวตรวจใน `.github/workflows/ci-compose.yml` ต้องยืนยันว่ากับดัก `${IMAGE_TAG:?...}` ยังอยู่ครบทั้ง 3 service (backend/frontend/bot_discord) เขียนด้วย `re.findall(r'\$\{IMAGE_TAG:\?([^}]*)\}', open('docker-compose.app.yml').read())` แล้ว assert `len(msgs) == 3`
- **Root Cause:** **คอมเมนต์ในไฟล์เดียวกันนั้นยกตัวอย่าง `${IMAGE_TAG:?...}` ไว้** (คำอธิบายกับดักที่เพิ่มใน PR #86/#87) ⇒ regex ทั้งไฟล์นับได้ **4** ทั้งที่กับดักจริงมี **3** ⇒ ตัวตรวจล้มทั้งที่ไฟล์ถูกต้อง 100%
  🔎 **นี่คือราคาของคอมเมนต์ที่ดี** — ยิ่งคอมเมนต์อธิบายแพตเทิร์นได้ชัดเท่าไร โอกาสที่ตัวตรวจแบบ "สแกนทั้งไฟล์" จะชนมันยิ่งสูง ⇒ **ไฟล์ที่เอกสารตัวเองได้ จะพังตัวตรวจแบบนี้เสมอ** ไม่ใช่เรื่องเฉพาะไฟล์นี้
- **Correct Pattern/Solution:** กรองเอา **เฉพาะบรรทัดที่ปฏิบัติการ** ก่อน แล้วค่อยจับแพตเทิร์น — ที่นี่คือ `re.match(r'\s*image:', l)` ⇒ ได้ 3 บรรทัดพอดี
  ```python
  img_lines = [l for l in lines if re.match(r'\s*image:', l)]
  if len(img_lines) != 3: sys.exit(1)          # จำนวน service ต้องครบ
  for l in img_lines:
      if not re.findall(r'\$\{IMAGE_TAG:\?([^}]*)\}', l): sys.exit(1)   # ทุกตัวต้องมีกับดัก
  ```
  ✅ **ได้ของแถมจากการเช็คแบบต่อบรรทัด:** ตรวจได้ทั้ง "กับดักหาย" และ "**service ใหม่ที่ไม่มีกับดัก**" ซึ่งแบบนับรวมทั้งไฟล์จับไม่ได้เลย (เพิ่ม service ที่ 4 แล้วนับได้ 4 = ผ่าน ทั้งที่ตัวใหม่ไม่มีกับดัก) — พอเพิ่มการเช็คจำนวนบรรทัด `image:` เป็น 3 ก็จับได้ทันที
- **Rule:** (1) 🔴 **ห้าม `grep`/regex ทั้งไฟล์บนไฟล์ที่คอมเมนต์อ้างโค้ดจริง** — ให้กรองบรรทัดที่ปฏิบัติการก่อนเสมอ (2) 🔴 **ตัวตรวจควรผูกกับ "บรรทัดที่ต้องมี" ไม่ใช่ "จำนวนที่เจอ"** — การนับรวมตอบไม่ได้ว่า *ตัวไหน* ขาด (3) ⚠️ **เขียนตัวตรวจแล้วต้องทดลองกับไฟล์ที่จงใจทำให้พังก่อน commit** ไม่งั้นความผิดพลาดจะย้ายจากโค้ดไปอยู่ในตัวตรวจ แล้ว CI จะเขียวตลอดกาลโดยไม่มีใครรู้ว่ามันตรวจอะไรอยู่
- **Tests:** 4 กรณีที่ต้องล้ม — ลบกับดักออกจาก `bot_discord` (`❌ บรรทัดนี้ไม่มีกับดัก`), ใส่ขีดในข้อความ error (`❌ ข้อความมีขีด`), เพิ่ม service ที่ 4 (`❌ บรรทัด image: มี 5 แต่ต้องมี 3`), และต้นฉบับที่ต้องผ่าน (`✅ กับดักครบ 3 จุด`)
- **Date Added:** 2026-09-26
