...
async def get_current_user_profile(current_user: dict = Depends(get_current_user), pool: asyncpg.Pool = Depends(get_db_pool)):
    ...
    async with pool.acquire() as conn:
        user = await conn.fetchrow("SELECT id, prefix, email, first_name, last_name, first_name_en, last_name_en, username, nickname, nickname_en, birthday, phone_number, line_id, address_house_no, address_road, address_sub_district, address_district, address_province, address_post_code, discord_id, google_id FROM users WHERE id = $1", user_id)
